...
export const ssurgo = async (
  ...
  const ssurgo1 = async () => {
    ...
    if (test('restrictions')) {
      attr.push(
        'reskind, resdept_l, resdept_r, resdept_h, resdepb_l, resdepb_r, resdepb_h, resthk_l, resthk_r, resthk_h',
      );
    }
    ...
    const joinComponent = test(
      'component|parentmaterial|restrictions|horizon|pores|structure|textureclass|canopycover|cropyield|monthlystats',
    )
  ...
  ((categories || '').match(/[ -][a-z]+/g) || []).forEach((cat) => {
    const t = cat.slice(1);
